    ...
    def get_provider_for_layer(self, layer: AnalysisLayer) -> Optional[BaseAIProvider]:
        ...
        for provider in self._providers.values():
            if provider.is_available():
                print(f"⚠️  Using last resort provider {provider.provider_name} for {layer.value}")
                return provider
        
        raise AIProviderError("Factory", f"No available providers for layer {layer.value}")
