...
class InvestmentBankingOptimizer:
    ...
    def optimize_investment_banking_multi_query(
        self, num_trials: int = 8
    ) -> InvestmentBankingMultiQueryModule:
        ...
        try:
            ...
            compiled_ib_multi_query = optimizer.compile(
                self.ib_multi_query, trainset=training_data, valset=training_data[:2]
            )
    ...
    def optimize_investment_banking_hyde(
        ...
        try:
            ...
            compiled_ib_hyde = optimizer.compile(
                self.ib_hyde, trainset=training_data, valset=training_data[:2]
            )
    ...
    def _investment_banking_hyde_metric(self, example, pred, trace=None) -> float:
        ...
        financial_score = sum(1 for term in financial_terms if term in doc_lower) / len(
            financial_terms
        )
        ...
        length_score = (
            min(word_count / 200.0, 1.0) if word_count >= 50 else word_count / 50.0
        )
        ...
        try:
            ...
            module_info = {
                "optimization_timestamp": (
                    str(dspy.get_current_time)
                    if hasattr(dspy, "get_current_time")
                    else "unknown"
                ),
                "modules_optimized": list(self.compiled_modules.keys()),
                "training_data_size": len(
                    self.create_investment_banking_training_data()
                ),
                "optimization_config": {
        ...
        try:
            ...
            if "ib_multi_query" in self.compiled_modules:
                ...
                for example in test_data:
                    pred = self.compiled_modules["ib_multi_query"].forward(
                        example.query
                    )
                    score = self._investment_banking_multi_query_metric(example, pred)
                    multi_query_scores.append(score)
                results["multi_query_avg_score"] = sum(multi_query_scores) / len(
                    multi_query_scores
                )
