...
async def observer_node(state: AxiomState) -> dict[str, Any]:
    ...
    try:
        ...
        validation_passed, validation_errors = validate_investment_banking_evidence(
            state
        )
        ...
        if not validation_passed and len(state["evidence"]) < 2:
            ...
            return {
                ...
                + [
                    HumanMessage(
                        content="Investment banking analysis failed validation - insufficient evidence for decision-making"
                    )
                ],
            }
        ...
        final_validation_passed, final_validation_errors = validate_investment_brief(
            brief
        )
...
Create comprehensive investment banking research brief:""",
    ...
    investment_confidence = calculate_investment_confidence(
        state["evidence"], state["task_plans"]
    )
...
def validate_investment_banking_evidence(state: AxiomState) -> tuple[bool, list[str]]:
    ...
    if (
        len(state["evidence"]) < settings.due_diligence_confidence_threshold * 10
    ):  # Scale threshold
        validation_errors.append(
            f"Insufficient evidence for investment decision: {len(state['evidence'])} pieces (minimum {int(settings.due_diligence_confidence_threshold * 10)} required)"
        )
    ...
    if len(high_confidence_evidence) < 3:
        validation_errors.append(
            "Insufficient high-confidence evidence for investment banking analysis"
        )
    ...
    if len(unique_sources) < 3:
        validation_errors.append(
            "Insufficient source diversity for reliable investment analysis"
        )
    ...
    financial_sources = [
        ...
        if any(
            domain in e.source_url
            for domain in ["sec.gov", "bloomberg", "reuters", "wsj", "ft.com"]
        )
    ]
    if not financial_sources:
        validation_errors.append(
            "No authoritative financial sources found - critical for investment banking decisions"
        )
...
def validate_investment_brief(brief: ResearchBrief) -> tuple[bool, list[str]]:
    ...
    if brief.confidence < settings.due_diligence_confidence_threshold:
        validation_errors.append(
            f"Investment confidence {brief.confidence:.2f} below threshold {settings.due_diligence_confidence_threshold}"
        )

    # Minimum content requirements
    if len(brief.key_findings) < 3:
        validation_errors.append(
            "Insufficient key findings for investment committee presentation"
        )

    if not brief.citations:
        validation_errors.append(
            "No citations provided - unacceptable for investment banking analysis"
        )

    # Evidence quality for investment decisions
    if len(brief.evidence) < 5:
        validation_errors.append(
            "Insufficient evidence depth for investment banking standards"
        )
