...
from axiom.models.base.base_model import BasePortfolioModel
...
allocation_logger = get_logger("axiom.models.portfolio.allocation")
...
class AllocationResult:
    ...
    asset_class_weights: Optional[Dict[str, float]] = None
    metrics: Optional[PortfolioMetrics] = None
    rebalancing_required: bool = False
...
class AssetAllocator(
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.logger = allocation_logger
        self.enable_logging = True
        self.optimizer = PortfolioOptimizer(config=self.portfolio_config)
        self.allocation_history: List[AllocationResult] = []
        
        if self.enable_logging:
            self.logger.info(
                "AssetAllocator initialized",
                n_asset_classes=len(self.asset_classes),
    ...
    @property
    def rebalancing_threshold(self) -> float:
        """Get rebalancing threshold (backward compatibility)."""
        return self.portfolio_config.rebalance_threshold
    ...
    def allocate(
        ...
        **kwargs
        ...
        """
        with self.track_time(f"asset_allocation_{strategy.value}"):
            ...
            if self.enable_logging:
                self.logger.info(
                    f"Allocation completed: {strategy.value}",
    ...
    def _risk_parity(self, returns: pd.DataFrame) -> np.ndarray:
        """Risk parity allocation using optimizer."""
        result = self.optimizer.optimize(
    ...
    def _black_litterman(
        ...
        # View uncertainty using config
        tau = self.portfolio_config.bl_tau
        Omega = np.diag([
        ...
        if self.portfolio_config.long_only:
    ...
    def _hierarchical_risk_parity(self, returns: pd.DataFrame) -> np.ndarray:
        ...
        # Recursive bisection
        weights = self._hrp_recursive_bisection(cov_matrix, sorted_indices)
    ...
    def _var_constrained(
        ...
        """VaR-constrained allocation using mixin."""
        ...
        # Bounds
        bounds = [(self.portfolio_config.min_weight, self.portfolio_config.max_weight)] * n_assets
        
        # Initial guess
        initial_weights = np.ones(n_assets) / n_assets
        
        # Optimize using mixin
        ...
        return optimal_weights if success else initial_weights
    ...
    def _check_rebalancing(
        ...
        """Check if rebalancing is required using config threshold."""
        for asset in current_weights.keys():
            current = current_weights.get(asset, 0)
            target = target_weights.get(asset, 0)
            
            if abs(current - target) > self.portfolio_config.rebalance_threshold:
                return True
    ...
    def rebalance(
        ...
        """
        Calculate rebalancing trades using config.
        
        Args:
            current_weights: Current portfolio weights
            target_weights: Target portfolio weights
            transaction_cost: Transaction cost per trade (uses config if None)
            
        Returns:
            Dictionary with rebalancing details
        """
        if transaction_cost is None:
            transaction_cost = self.portfolio_config.transaction_costs
    ...
    def optimize(self, **kwargs):
        """Implement required abstract method (delegates to allocate)."""
