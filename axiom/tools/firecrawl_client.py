...
class FirecrawlClient:
    ...
    async def scrape_sec_filing(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = await loop.run_in_executor(
                None,
                lambda: self.client.scrape_url(url=url, params=sec_params)
            )
    ...
    async def crawl_investor_relations(
        ...
        if focus_areas is None:
            focus_areas = ["earnings", "financial", "investor", "reports", "sec-filings"]
        ...
        try:
            crawl_params = {
                ...
                "excludeTags": [
                    "nav", "footer", ".ads", ".social", "iframe"
                ],
                "crawlerOptions": {
                    "includes": [f"*{area}*" for area in focus_areas]
                }
            }
            ...
            response = await loop.run_in_executor(
                None,
                lambda: self.client.crawl_url(url=base_url, params=crawl_params)
            )
    ...
    async def extract_financial_tables(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = await loop.run_in_executor(
                None,
                lambda: self.client.scrape_url(url=url, params=table_params)
            )
    ...
    def _is_financial_document(self, url: str) -> bool:
        ...
        financial_indicators = [
            'sec.gov', 'edgar', 'investor', 'earnings', 'financial',
            '10-k', '10-q', '8-k', 'annual-report', 'quarterly'
        ]
        
        url_lower = url.lower()
        return any(indicator in url_lower for indicator in financial_indicators)

    def _enhance_financial_content(self, response: Dict[str, Any], url: str) -> Dict[str, Any]:
        """Enhance scraped content with financial document processing."""
        
        if 'markdown' in response:
            # Add financial document metadata
            response['financial_metadata'] = {
                'document_type': self._detect_document_type(url, response.get('markdown', '')),
                'extraction_timestamp': asyncio.get_event_loop().time(),
                'source_authority': self._assess_source_authority(url),
                'contains_tables': 'table' in response.get('html', '').lower(),
                'financial_keywords': self._extract_financial_keywords(response.get('markdown', ''))
            }

        return response

    def _extract_sec_metadata(self, response: Dict[str, Any], url: str) -> Dict[str, Any]:
    ...
    def _process_investor_crawl_results(self, response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _assess_source_authority(self, url: str) -> str:
        ...
        if 'sec.gov' in url_lower:
            return 'regulatory'
        elif any(domain in url_lower for domain in ['bloomberg.com', 'reuters.com', 'wsj.com']):
    ...
    def _extract_financial_keywords(self, content: str) -> List[str]:
        ...
        financial_keywords = [
            'revenue', 'ebitda', 'net income', 'cash flow', 'debt',
            'valuation', 'dcf', 'multiples', 'enterprise value',
            'merger', 'acquisition', 'm&a', 'synergies'
        ]
