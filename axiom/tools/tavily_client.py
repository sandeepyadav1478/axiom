...
class TavilyClient:
    ...
    def __init__(self):
        ...
        self.financial_domains = [
            ...
            'fool.com', 'seekingalpha.com', 'zacks.com',
    ...
    async def search(
        ...
        try:
            ...
            if response and 'results' in response:
                response['results'] = self._rank_financial_results(response['results'], query)
    ...
    async def company_search(self, company_name: str, analysis_type: str = "overview") -> Optional[Dict[str, Any]]:
    ...
    async def sector_search(self, sector: str, focus: str = "trends") -> Optional[Dict[str, Any]]:
    ...
    async def qna_search(self, query: str) -> Optional[str]:
        ...
        try:
            ...
            response = await loop.run_in_executor(
                None,
                lambda: self.client.qna_search(query=enhanced_query)
            )
    ...
    def _enhance_financial_query(self, query: str) -> str:
        ...
        financial_terms = {
            ...
            'performance': ['financial performance', 'EBITDA', 'revenue growth', 'profitability'],
        ...
        if enhancements:
            enhanced_query = f"{query} {' '.join(enhancements[:3])}"  # Limit enhancements
        else:
    ...
    def _parse_time_range(self, time_range: str) -> Optional[int]:
        ...
        time_mappings = {
            '1d': 1, '1w': 7, '2w': 14, '1m': 30, '3m': 90,
            '6m': 180, '1y': 365, '2y': 730
        }
        
        return time_mappings.get(time_range.lower())

    def _rank_financial_results(self, results: List[Dict], original_query: str) -> List[Dict]:
        ...
        def calculate_financial_score(result: Dict) -> float:
            ...
            financial_keywords = ['financial', 'earnings', 'revenue', 'valuation',
                                'm&a', 'merger', 'acquisition', 'ebitda', 'dcf']
            keyword_boost = sum(0.1 for keyword in financial_keywords
                               if keyword in title or keyword in content[:500])
            
            # Boost recent content (simple heuristic)
            recency_boost = 0.1 if any(term in content for term in ['2024', '2023', 'recent', 'latest']) else 0
