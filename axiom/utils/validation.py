...
from .error_handling import FinancialDataError, ComplianceError, AxiomError, ErrorSeverity
...
class FinancialValidator:
    ...
    INDUSTRY_RULES = {
        ...
        "banking": {
            "min_tier1_capital_ratio": 6.0,
            "max_loan_to_deposit": 100.0
        }
    }

    @staticmethod
    def validate_financial_metrics(metrics: Dict[str, Union[float, int]], 
                                 industry: Optional[str] = None) -> List[str]:
        ...
        for metric, value in metrics.items():
            ...
            if metric in FinancialValidator.RATIO_RANGES:
                ...
                if not (min_val <= float_value <= max_val):
                    errors.append(f"{metric} value {float_value} outside normal range ({min_val}-{max_val})")
                ...
                for rule, threshold in industry_rules.items():
                    if rule.startswith("min_") and metric == rule[4:]:
                        if float_value < threshold:
                            errors.append(f"{metric} {float_value} below industry minimum {threshold}")
                    elif rule.startswith("max_") and metric == rule[4:]:
                        if float_value > threshold:
                            errors.append(f"{metric} {float_value} above industry maximum {threshold}")
    ...
    def validate_ma_transaction(transaction_data: Dict[str, Any]) -> List[str]:
        ...
        errors = []
        required_fields = ["target_company", "acquirer", "transaction_value", "announcement_date"]
...
class ComplianceValidator:
    ...
    @staticmethod
    def validate_confidence_levels(analysis: Dict[str, Any], analysis_type: str) -> List[str]:
        """Validate confidence levels meet compliance requirements."""
        
        errors = []
        required_threshold = ComplianceValidator.CONFIDENCE_THRESHOLDS.get(analysis_type, 0.7)
        ...
        if "evidence" in analysis:
            ...
            if len(evidence_list) < 3:
                errors.append(f"Insufficient evidence for {analysis_type} (minimum 3 required)")
            
            # Check evidence sources
            authoritative_sources = 0
            for evidence in evidence_list:
                if any(domain in evidence.get("source_url", "").lower() 
...
class DataQualityValidator:
    ...
    @staticmethod
    def validate_search_results(results: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    def validate_evidence_quality(evidence: List[Dict[str, Any]]) -> List[str]:
        ...
        for item in evidence:
            if "source_url" in item:
                domain = item["source_url"].split("/")[2] if "/" in item["source_url"] else item["source_url"]
                sources.add(domain)
        ...
        low_confidence_count = sum(1 for item in evidence 
        ...
        financial_terms = ["revenue", "profit", "ebitda", "valuation", "market", "financial"]
        relevant_evidence = 0
...
def validate_investment_banking_workflow(workflow_data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    try:
        ...
        if "financial_metrics" in workflow_data:
            validation_results["financial_metrics"] = FinancialValidator.validate_financial_metrics(
                workflow_data["financial_metrics"],
                workflow_data.get("industry")
            )
        
        # Validate company data
        if "company_data" in workflow_data:
            validation_results["company_data"] = FinancialValidator.validate_company_data(
                workflow_data["company_data"]
            )
        ...
        if "evidence" in workflow_data:
            validation_results["data_quality"] = DataQualityValidator.validate_evidence_quality(
                workflow_data["evidence"]
            )
        
        # Count total errors
        total_errors = sum(len(errors) for errors in validation_results.values() if isinstance(errors, list))
        
        if total_errors > 0:
            validation_results["overall_errors"].append(f"Total validation errors: {total_errors}")
    
    except Exception as e:
        validation_results["overall_errors"].append(f"Validation process error: {str(e)}")
    
    return validation_results


def raise_validation_errors(validation_results: Dict[str, List[str]], severity: ErrorSeverity = ErrorSeverity.HIGH):
